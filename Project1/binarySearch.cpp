...
int binary_search_for_m(vector<long>& sum, long sum_i, int LOWER, int low, int high) {
	...
	*/
		...
		if (sum[index] - sum_i >= LOWER)
			return binary_search_for_m(sum, sum_i, LOWER, low, index + 1);
		else
			return binary_search_for_m(sum, sum_i, LOWER, index+1, high);
...
int binary_search_for_n(vector<long>& sum, long sum_i, int UPPER, int low, int high) {
	...
	*/
		...
		if (sum[index] - sum_i >UPPER)
			return binary_search_for_m(sum, sum_i, UPPER, low, index + 1);
		else
			return binary_search_for_m(sum, sum_i, UPPER, index+1, high);
